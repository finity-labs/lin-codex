# Codex help drawer (Vue)

Published by `php artisan vendor:publish --tag=lin-codex-vue` into `resources/js/codex`. The files are yours now: the package never loads them, ships no npm package and will not overwrite them unless you publish again with `--force`.
...
| `HelpButton.vue` | A button that opens the drawer. |
| `HelpDrawer.vue` | The drawer: the current page's articles, search, the tree and the article view. |

`--tag=lin-codex-react` publishes the React set into the same folder. The two tags are alternatives; `codex.ts` and `types.ts` are identical in both, so publishing both overwrites the shared files with the same content and leaves both component pairs side by side.
...
import HelpButton from '@/codex/HelpButton.vue'
import HelpDrawer, { openCodex } from '@/codex/HelpDrawer.vue'

const page = usePage()
</script>

<template>
  <HelpButton @click="openCodex()" />
  <HelpDrawer :prefix="page.props.codex.prefix" :context="page.props.codex.context" />
  <slot />
</template>
```

In a template without `usePage()`, `$page.props.codex.context` and `$page.props.codex.prefix` work the same way.
