...
The drawer opens on the button, on `Ctrl+/`, on a `codex:open` window event and on a `?codex=slug` query parameter, read once on mount. Without a slug it opens on the first article for the current page; with one it opens that article. `openCodex(slug?, heading?)` dispatches the event for you:
